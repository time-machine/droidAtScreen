...
public class DevicePane extends JPanel {
  ...
  public DevicePane(DeviceFrame deviceFrame, AndroidDevice dev,
      boolean portrait, int scalePercentage, int frameRate) {
  ...
  public void setScalePercentage(int scalePercentage) {
    if (scalePercentage < 10 || 300 < scalePercentage) {
      throw new IllegalArgumentException("Scale % is invalid: " +
          scalePercentage);
    }
  ...
  private void setFrameRate(int rate) {
    if (rate <= 0) {
      throw new IllegalArgumentException("Frame rate must be a positive " +
          "number. Value = " + rate);
    }
    this.frameRate = rate;
  }
