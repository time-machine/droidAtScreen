...
public class ScaleImageTransformer implements ImageTransformer {
   ...
   * @param scale a postive real number (1 = no scaling)
   */
  public void setScale(double scale) {
    if (scale <= 0) {
      throw new IllegalArgumentException("Transformation scale must be a " +
          "positive real number. Illegal value = " + scale);
    }
    this.scale = scale;
  }
